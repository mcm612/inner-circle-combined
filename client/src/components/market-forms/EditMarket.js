...
import { getMarketById, updateMarketById } from '../../actions/market';
...
const EditMarket = ({ market: { marketbyid, loading }, getMarketById, updateMarketById, match, history }) => {
	...
	useEffect(
		() => {
			if (!marketbyid) getMarketById(match.params.id);
			if (!loading) {
				const marketData = { ...initialState };
				for (const key in marketbyid) {
					if (key in marketData) marketData[key] = marketbyid[key];
				}
				setFormData(marketData);
			}
		},
		[ match.params.id, marketbyid, loading, getMarketById ]
	);
	...
	const onSubmit = (e) => {
		...
		updateMarketById(match.params.id, newFormData);
	};
	return (
		<Fragment>
			<h1 className="large text-primary mobile-margin">Edit Market Area</h1>
				...
				<form className="form" onSubmit={(e) => onSubmit(e)}>
					...
					<div className="form-group">
						<div className="form__photo-upload m-1-bottom">
							<img
								className="form__market-photo"
								src={!market_photo ? defaultImg : market_photo}
								alt={location}
							/>
							<input
					...
					<Link to="/dashboard-market" className="btn btn-light my-1">
						Go Back
					</Link>
				</form>
			</div>
...
export default connect(mapStateToProps, { getMarketById, updateMarketById })(withRouter(EditMarket));
